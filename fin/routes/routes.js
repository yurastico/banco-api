...
router.get("/",auth, (req,res) => {
    ...
    }).catch((error) => res.status(500).send({output: "internal error", error: error}))
})

router.get("/find",auth, (req,res) => {
    Account.find().then((result) => {
        res.status(200).send({output: "ok", payload: result})
    }).catch((error) => res.status(500).send({output: "internal error", erro: error}))
}) 
...
router.put("/update/:id",auth,(req,res)=> {
    Account.findByIdAndUpdate(req.params.id,
        ...
        res.status(202).send({output:"updated", id: req.params.id, payload: result})
    }).catch((error)=> {
