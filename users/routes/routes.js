...
router.post('/login',(req,res)=> { 
    ...
    User.findOne({username:us}).then((data)=> {
        ...
        bcrypt.compare(ps,data.password,(error,same)=>{
            ...
            if (!same) return res.status(400).send({output: "Authentication Fail"})
            const token = createToken(data._id,data.user)
            const info = new ManagerUser({useId: data._id,username:data.username,information: req.headers})
            info.save()
            res.status(200).send({output: "Authorized",payload: data,token:token})
